...
TARGET_DIR = r"C:\temp\QuarterCuboid_Results" #replace with target directory
...
TOTAL_RUNS = 500
CSV_FILENAME = 'WedgeBilayer_Results.csv'
MODEL_NAME_PREFIX = 'Wedge_2D_'
...
if __name__ == '__main__':
    with open(CSV_FILENAME, 'w') as csvfile:
        ...
        writer.writerow(['Run_ID', 'Length_mm', 'TPU_Thick_mm', 'PLA_Thick_mm', 'Total_Thick_mm', 'Shrinkage_Strain', 'Deflection_mm'])

        for i in range(1, TOTAL_RUNS + 1):
            p_len = random.uniform(75.0, 125.0)
            p_t_tpu = random.uniform(0.15, 0.25)
            p_t_pla = 3.0 * p_t_tpu
            ...
            try:
                ...
                if res is not None:
                    writer.writerow([i, round(p_len, 2), round(p_t_tpu, 2), round(p_t_pla, 2), round(total_t, 2), round(current_shrinkage, 4), round(res, 4)])
