...
# CONFIGURATION
TOTAL_RUNS = 200
CSV_FILENAME = 'Bimetal_Dataset_200Runs_NewDataset.csv'
MODEL_NAME_PREFIX = 'Model_PLA_TPU_'

# Material Properties
# TPU (Passive Layer - Bottom)
MAT_TPU_E = 25.0  # Modulus (MPa)
MAT_TPU_NU = 0.48  # Poisson's Ratio
MAT_TPU_ALPHA = 6.8e-5  # Lower Thermal Expansion Coefficient

# PLA (Active Layer - Top)
MAT_PLA_E = 350.0  # Modulus at Tg (MPa)
MAT_PLA_NU = 0.35  # Poisson's Ratio
MAT_PLA_ALPHA = 1.5e-4  # Higher Thermal Expansion Coefficient
...
if __name__ == '__main__':
    # Fixed file writing code to avoid "bytes-like object" and "illegal newline" errors
    with open(CSV_FILENAME, 'w') as csvfile:
        ...
        for i in range(1, TOTAL_RUNS + 1):
            ...
            p_t_tpu = random.uniform(0.2, 1.5)  # Passive thickness
            p_t_pla = random.uniform(0.2, 1.5)  # Active thickness

            ratio = p_t_pla / (p_t_tpu + p_t_pla)  # PLA fraction of total thickness

            # Heating to 60°C to simulate hitting the Tg limit
            p_temp = 60.0
            ...
            try:
                ...
                if result is not None:
                    writer.writerow([i, round(p_len, 2), round(p_t_tpu, 2), round(p_t_pla, 2), round(ratio, 2), p_temp,
                                     round(result, 4)])
                    print("--> Success! Deflection: {:.4f} mm".format(result))
                else:
