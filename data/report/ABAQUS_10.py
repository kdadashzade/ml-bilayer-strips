...
def create_and_run(run_id, length, t_tpu, t_pla, temp_change):
    ...
    # t_tpu is now the maximum thickness (the peak) of the bottom layer
    total_h = t_tpu + t_pla
    ...
    # Partition: Creating the Wedge Shape
    f = part.faces
    ...
    # Draw from bottom-left to center-peak, then center-peak to bottom-right
    s_part.Line(point1=(0.0, 0.0), point2=(length / 2.0, t_tpu))
    s_part.Line(point1=(length / 2.0, t_tpu), point2=(length, 0.0))
    ...
    # TPU (Bottom Wedge) - Point safely inside the triangle at X = L/2
    ...
    # PLA (Top Layer) - Point safely in the upper half at X = L/2
    ...
    # Optional: Use QUAD_DOMINATED to better handle the sharp wedge corners
    ...
    edges = inst.edges
    # Because the partition starts exactly at (0,0), the left edge is no longer split.
    # We find the single edge at the midpoint of the total height.
    left_edges = edges.findAt(((0.0, total_h / 2.0, 0.0),))
    ...
    model.Temperature(name='Temp-Init', createStepName='Initial', region=region_all, distributionType=UNIFORM, magnitudes=(0.0,))
    model.Temperature(name='Temp-Final', createStepName='Step-Heat', region=region_all, distributionType=UNIFORM, magnitudes=(temp_change,))
...
if __name__ == '__main__':
    with open(CSV_FILENAME, 'w') as csvfile:
        ...
        writer.writerow(['Run_ID', 'Length_mm', 'TPU_Thick_Peak_mm', 'PLA_Thick_Min_mm', 'PLA_Ratio', 'Temp_Change_C', 'Deflection_mm'])
        ...
        for i in range(1, TOTAL_RUNS + 1):
            ...
            try:
                ...
                if result is not None:
                    writer.writerow([i, round(p_len, 2), round(p_t_tpu, 2), round(p_t_pla, 2), round(ratio, 2), p_temp, round(result, 4)])
